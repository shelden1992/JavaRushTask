...
public class Solution {
    private static final int NUMBER_OF_THREADS=3; //3 треда будет обрабатывать нашу очередь
    private static final int MAX_BATCH_SIZE=100; //будем вытаскивать по 100 сообщений

    private Logger logger=Logger.getLogger(Solution.class.getName());
    private BlockingQueue <Integer>messageQueue=new LinkedBlockingQueue<>();//тут будут храниться все сообщения
            ...
            new Thread() {
                ...
                public   void run() {
                        ...
                        while (true) {
                            try {
//                                this.wait();
                                messageQueue.drainTo(batch, MAX_BATCH_SIZE);  //Removes at most the given number of available elements
